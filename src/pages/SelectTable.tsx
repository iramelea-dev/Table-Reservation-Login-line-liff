...
interface TableNode {
  ...
  size: Tabletype;
  shape: Tableshape;
  sizeRatio: number; // ขนาดสเกลภาพบน canvas
  /** group fields */
  isGroup?: boolean;           // เป็นโหนดกรุ๊ปแทนโต๊ะเดี่ยว
  members?: TableNode[];       // โต๊ะที่รวมอยู่ในกรุ๊ป (เก็บสnapshot เดิมไว้ คืนได้)
}
...
const Selecttable: React.FC = () => {
  ...
  const confirmJoin = () => {
    ...
    const members = tables.filter(t => ids.includes(t.id));

    // สร้างค่ากลาง ๆ สำหรับกรุ๊ป
    const sumX = members.reduce((s, m) => s + m.x, 0);
    ...
    // ขนาดกรุ๊ป = max ของสมาชิก เพื่อไม่ทับกันเกินไป
    const sizeRatio = Math.max(...members.map(m => m.sizeRatio ?? TABLE_VISUAL.MEDIUM));

    const groupNode: TableNode = {
      id: guid(),
      label: `G${Math.floor(Math.random() * 900 + 100)}`, // label กรุ๊ปแบบง่าย
      ...
      sizeRatio,
      isGroup: true,
      members: JSON.parse(JSON.stringify(members)), // snapshot เดิมไว้คืน
    };

    // ลบสมาชิกที่เลือกออก แล้วใส่กรุ๊ปแทน
  ...
  const splitSelectedGroup = () => {
    ...
    // นำสมาชิกเดิมกลับมา และลบโหนดกรุ๊ป
    setTables(prev =>
      prev
  ...
  const onPointerDown = (e: React.PointerEvent, item: TableNode | ObjectNode) => {
    ...
    if (joinMode) return; // ไม่ให้ลากตอนรวมโต๊ะอยู่
    const rect = containerRef.current.getBoundingClientRect();
              ...
              <>
                {!joinMode ? (
                  <>
                    ...
                    <IonButton
                      color="warning"
                      disabled={!selected || !("zone" in (selected || {})) || !(selected as TableNode).isGroup}
        ...
        >
          ...
          {objects.map((o) => (
            <div
              ...
              onClick={(e) => { e.stopPropagation(); if (!editMode) return; setSelected(o); }}
              title={o.label}
              style={{
                position: "absolute",
                left: `${o.x * 100}%`,
            ...
            return (
              <div
                ...
                style={{
                  ...
                  // ✅ กลมเป๊ะ: กำหนดเฉพาะ width + บังคับสัดส่วนเป็น 1:1
                  width: `${sidePct}%`,
                  aspectRatio: "1 / 1",     // <-- สำคัญ: ทำให้สูง=กว้างเสมอ
                  boxSizing: "border-box",   // รวม border/padding ในกรอบ
                  ...
                  // ❌ อย่าใส่ padding ที่ชั้นนี้ (เพี้ยนวงกลม)
                  // padding: "8px 12px",
                  display: "block",
                }}
              >
                {/* ✅ ย้าย padding/จัดกึ่งกลางมาที่ inner wrapper แทน */}
                <div
                  style={{
                    position: "absolute",
                    inset: 0,
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    padding: 8,            // padding อยู่ชั้นใน ไม่ทำวงกลมเพี้ยน
                    textAlign: "center",
                  }}
                >
                  {t.label}
          ...
          <IonContent style={{ padding: 16 }}>
            {selected && "zone" in selected ? (
              <>
                ...
                {selected.isGroup && selected.members?.length ? (
                  <div style={{ marginTop: 12, fontSize: 13, opacity: .85 }}>
                    สมาชิก: {selected.members.map(m => m.label).join(", ")}
                  </div>
                ) : null}
